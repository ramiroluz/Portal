...
const View = (props) => {
    ...
    const linksDestaques = data?.linksDestaques?.filter((z) => z?.link !== undefined) || [];
    const linksMaisLidos = data?.linksMaisLidos?.filter((z) => z?.link !== undefined) || [];
                    ...
                    <div className="stack gap-24">
                        ...
                        <div className="destaques grid-col-2 gap-24">
                            {links.slice(1, 30).map((link, index) => (
                                <Link key={index} className="card-default py-24 px-24"
                                      to={link?.link[0]["getPath"].replace("/Plone/", "")}>
                                    <div>
                                        {link?.link[0]?.image_scales?.image[0]?.download && (
                                            <img
                                        ...
                                        <span className="tag-color inline-block mt-24">Sem categoria</span>
                                        <h2 className="title-32 mt-24">
                                            {link?.link[0]?.title}
                                        </h2>
                        ...
                        <div>
                            ...
                            <div className="stack gap-24">
                                {linksDestaques?.map((link, index) => (
                                    <Link key={index} className="card-default py-24 px-24 mt-24"
                                          to={link?.link[0]["getPath"].replace("/Plone/", "")}>
                                        <div>
                                            {link?.link[0]?.image_scales?.image[0]?.download && (
                                                <img
