            ...
            <div className="grid-col-3 gap-30 flex-column-mb flex-mb">
              {
                noticias?.slice(0, 3).map((item, key) =>
                  <Link className="item card-default py-24 px-24 destaque stack gap-24" to={flattenToAppURL(item["@id"])}>
                    <div className="thubmnail">
                      <img
                        src={flattenToAppURL(item["@id"] + "/" + item.image_scales?.image[0]?.download) || "/images/news/default.png"}
                        alt={item?.title} className="aspect-ratio-16-9"/>
                    </div>
                    {/*<div>*/}
